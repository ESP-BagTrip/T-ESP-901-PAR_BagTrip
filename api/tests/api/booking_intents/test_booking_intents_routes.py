...
class TestCreateBookingIntent:
    ...
    def test_create_booking_intent_success(self, mock_service, client, override_get_current_user, override_get_db, mock_booking_intent):
    ...
    @patch("src.api.booking_intents.routes.BookingIntentsService")
    def test_create_booking_intent_error(self, mock_service, client, override_get_current_user, override_get_db):
...
class TestBookBookingIntent:
    ...
    def test_book_hotel_success(self, mock_orchestrator, client, override_get_current_user, override_get_db, mock_booking_intent):
        ...
        assert response.status_code == 201
        data = response.json()
        assert data["amadeus"]["type"] == "hotel"
        assert data["amadeus"]["bookingId"] == "HOTEL_123"
    ...
    def test_book_unknown_type(self, mock_orchestrator, client, override_get_current_user, override_get_db, mock_booking_intent):
        """Test booking with unknown intent type (coverage)."""
