...
class TestTravelersService:
    ...
    def test_create_traveler_success(self, mock_get_trip, mock_db_session):
    ...
    @patch("src.services.travelers_service.TripsService.get_trip_by_id")
    def test_create_traveler_trip_not_found(self, mock_get_trip, mock_db_session):
        """Test error when trip not found."""
        mock_get_trip.return_value = None
        
        with pytest.raises(AppError) as exc:
            TravelersService.create_traveler(
                mock_db_session, uuid.uuid4(), uuid.uuid4(), None, "ADULT", "John", "Doe"
            )
        assert exc.value.code == "TRIP_NOT_FOUND"

    @patch("src.services.travelers_service.TripsService.get_trip_by_id")
    def test_get_travelers_by_trip(self, mock_get_trip, mock_db_session):
        ...
        result = TravelersService.get_travelers_by_trip(
            mock_db_session, trip_id, uuid.uuid4()
        )
        assert len(result) == 1

    @patch("src.services.travelers_service.TripsService.get_trip_by_id")
    def test_get_traveler_by_id(self, mock_get_trip, mock_db_session):
        ...
        result = TravelersService.get_traveler_by_id(
            mock_db_session, mock_traveler.id, trip_id, uuid.uuid4()
        )
        assert result == mock_traveler

    @patch("src.services.travelers_service.TravelersService.get_traveler_by_id")
    def test_update_traveler_success(self, mock_get_traveler, mock_db_session):
        ...
        result = TravelersService.update_traveler(
            mock_db_session, uuid.uuid4(), uuid.uuid4(), uuid.uuid4(), first_name="New"
        )
    ...
    @patch("src.services.travelers_service.TravelersService.get_traveler_by_id")
    def test_update_traveler_not_found(self, mock_get_traveler, mock_db_session):
    ...
    @patch("src.services.travelers_service.TravelersService.get_traveler_by_id")
    def test_delete_traveler_success(self, mock_get_traveler, mock_db_session):
        """Test successful traveler deletion."""
        mock_traveler = TripTraveler(id=uuid.uuid4())
        mock_get_traveler.return_value = mock_traveler
        
        TravelersService.delete_traveler(
            mock_db_session, uuid.uuid4(), uuid.uuid4(), uuid.uuid4()
        )
