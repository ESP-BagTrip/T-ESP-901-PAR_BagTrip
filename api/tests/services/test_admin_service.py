...
class TestAdminService:
    ...
    def test_get_all_hotel_bookings(self, mock_db_session):
        """Test retrieving all hotel bookings."""
        ...
        mock_query = mock_db_session.query.return_value
        mock_query.join.return_value.join.return_value.join.return_value.order_by.return_value.count.return_value = 1
        mock_query.join.return_value.join.return_value.join.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [(booking, trip_title, user_email, hotel_id)]

        items, total, total_pages = AdminService.get_all_hotel_bookings(mock_db_session)

        assert len(items) == 1
        assert items[0]["status"] == "CONFIRMED"
        assert items[0]["hotel_id"] == "HOTEL123"
