...
describe('UserPill', () => {
  describe('initials extraction', () => {
    it('extracts initials from two-part email local (dot separator)', () => {
      renderWithProviders(
        <UserPill user={makeUser({ email: 'john.doe@test.com' })} collapsed={false} />
      )
    ...
    it('extracts initials from two-part email local (dash separator)', () => {
      renderWithProviders(
        <UserPill user={makeUser({ email: 'jane-smith@test.com' })} collapsed={false} />
      )
    ...
    it('extracts initials from two-part email local (underscore separator)', () => {
      renderWithProviders(
        <UserPill user={makeUser({ email: 'bob_ross@test.com' })} collapsed={false} />
      )
    ...
    it('extracts first two chars when single-part email local', () => {
      renderWithProviders(
        <UserPill user={makeUser({ email: 'admin@test.com' })} collapsed={false} />
      )
  ...
  describe('collapsed state', () => {
    it('renders initials in avatar', () => {
      renderWithProviders(
        <UserPill user={makeUser({ email: 'john.doe@test.com' })} collapsed={true} />
      )
      expect(screen.getByText('JD')).toBeInTheDocument()
    })
    ...
    it('does not show email as direct paragraph', () => {
      ...
      const emailParagraph = Array.from(paragraphs).find(
        p => p.textContent === 'john.doe@example.com'
      )
