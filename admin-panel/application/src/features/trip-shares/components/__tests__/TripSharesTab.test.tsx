...
describe('TripSharesTab', () => {
  it('renders DataTable', () => {
    render(<TripSharesTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useTripSharesTab } = await import('../../hooks')
    vi.mocked(useTripSharesTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<TripSharesTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useTripSharesTab } = await import('../../hooks')
    vi.mocked(useTripSharesTab).mockReturnValue({
