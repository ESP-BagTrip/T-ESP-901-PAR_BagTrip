...
describe('UsersTab', () => {
  it('renders DataTable', () => {
    render(<UsersTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useUsersTab } = await import('../../hooks')
    vi.mocked(useUsersTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<UsersTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useUsersTab } = await import('../../hooks')
    vi.mocked(useUsersTab).mockReturnValue({
      data: { data: [], pagination: { page: 1, limit: 10, total: 0, total_pages: 0 } },
      isLoading: false,
      setPage: mockSetPage,
    } as any)
    render(<UsersTab isActive={true} />)
