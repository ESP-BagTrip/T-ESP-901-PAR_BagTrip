...
describe('FlightsTab', () => {
  it('renders DataTable', () => {
    render(<FlightsTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useFlightsTab } = await import('../../hooks')
    vi.mocked(useFlightsTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<FlightsTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useFlightsTab } = await import('../../hooks')
    vi.mocked(useFlightsTab).mockReturnValue({
