...
describe('FeedbacksTab', () => {
  it('renders DataTable', () => {
    render(<FeedbacksTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useFeedbacksTab } = await import('../../hooks')
    vi.mocked(useFeedbacksTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<FeedbacksTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useFeedbacksTab } = await import('../../hooks')
    vi.mocked(useFeedbacksTab).mockReturnValue({
