...
describe('ActivitiesTab', () => {
  it('renders DataTable', () => {
    render(<ActivitiesTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useActivitiesTab } = await import('../../hooks')
    vi.mocked(useActivitiesTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<ActivitiesTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useActivitiesTab } = await import('../../hooks')
    vi.mocked(useActivitiesTab).mockReturnValue({
