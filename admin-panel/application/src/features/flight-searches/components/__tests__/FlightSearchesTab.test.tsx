...
import FlightSearchesTab from '../FlightSearchesTab'

describe('FlightSearchesTab', () => {
  it('renders DataTable', () => {
    render(<FlightSearchesTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useFlightSearchesTab } = await import('../../hooks')
    vi.mocked(useFlightSearchesTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<FlightSearchesTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useFlightSearchesTab } = await import('../../hooks')
    vi.mocked(useFlightSearchesTab).mockReturnValue({
