...
describe('BudgetItemsTab', () => {
  it('renders DataTable', () => {
    render(<BudgetItemsTab isActive={true} />)
    expect(screen.getByTestId('data-table')).toBeInTheDocument()
  })

  it('passes isLoading to DataTable', async () => {
    const { useBudgetItemsTab } = await import('../../hooks')
    vi.mocked(useBudgetItemsTab).mockReturnValue({ data: null, isLoading: true, setPage: vi.fn() } as any)
    render(<BudgetItemsTab isActive={true} />)
  ...
  it('calls setPage on pagination change', async () => {
    ...
    const { useBudgetItemsTab } = await import('../../hooks')
    vi.mocked(useBudgetItemsTab).mockReturnValue({
