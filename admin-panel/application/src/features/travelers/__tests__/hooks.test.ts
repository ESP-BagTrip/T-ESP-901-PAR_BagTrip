...
vi.mock('@/services', () => ({
  adminService: { getAllTravelers: vi.fn() },
}))
...
describe('useTravelersTab', () => {
  ...
  it('should call usePaginatedQuery with correct queryKey', async () => {
    const { useTravelersTab } = await import('@/features/travelers/hooks')
    useTravelersTab({ enabled: true })
  ...
  it('should use adminService.getAllTravelers as queryFn', async () => {
    const { useTravelersTab } = await import('@/features/travelers/hooks')
    useTravelersTab({ enabled: true })
    ...
    expect(adminService.getAllTravelers).toHaveBeenCalledWith(params)
  })
})
